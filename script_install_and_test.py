...
def install_apps():
    swipe_screen()
    output = subprocess.Popen("adb install 'Área de Trabalho'/teste/apks/wikipedia.apk", shell=True, stdout=subprocess.PIPE)
    output = subprocess.Popen("adb install 'Área de Trabalho'/teste/apks/ankidroid.apk", shell=True, stdout=subprocess.PIPE)
    time.sleep(5)
    output = subprocess.Popen("adb shell input keyevent 4", shell=True, stdout=subprocess.PIPE)
